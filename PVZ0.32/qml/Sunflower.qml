...
Plant{
    ...
    BoxCollider{
        ...
        fixture.onBeginContact: {
            attached = true
            sunflower.beZombies.push(other.getBody().target)
            sunflower.beZombiesNumber+=1
        }
    }
    ...
    onCurrentBloodChanged: {
        if(currentBlood == 0){
            for(var i = 0;i < sunflower.beZombiesNumber;i++){
                if(sunflower.beZombies[i] != null){
                    sunflower.beZombies[i].attact = false
                }
            }
            sunflower.beZombiesNumber = 0
            lifeReduction.stop()
            field.deletePlant()
        }
    }
